...
function init() {
  document.title = `${topic.title} | Cartesian Plane Laboratory`;
...
function renderSliders(container, state, context) {
  ...
  topic.params.forEach(param => {
    ...
    if (param.guide) {
      ...
      swatch.className = `param-swatch guide-${param.guide}`;
      swatch.setAttribute("aria-hidden", "true");
      label.append(swatch);
    }

    label.append(document.createTextNode(param.label));
...
function render() {
  graph.setOptions({
    ...
    showGuides: dom.showGuides.checked
  });
  ...
  if (mode === "explore") {
    ...
    graph.render(topic, challengeParams, { targetParams, suppressGuides: true });
  }
}
...
function bindEvents() {
  ...
  dom.showGuides.addEventListener("change", render);
...
function switchMode(nextMode) {
  ...
  dom.showGuides.disabled = mode === "challenge";
  clearFeedback();
  render();
}
...
function checkChallenge() {
  ...
  if (exact) {
    ...
    dom.feedback.innerHTML = `Correct. You found all the parameters: <strong>${topic.formula(targetParams)}</strong>`;
    return;
  }

  dom.feedback.className = "feedback warning";
  dom.feedback.textContent = close
    ? "Almost there. At least one parameter is still slightly different."
    : `Not yet. ${buildHint()}`;
}

function buildHint() {
  if (topic.key === "line") {
    if (challengeParams.q !== targetParams.q) return "Look first at where the line crosses the y-axis.";
    return "Now compare the slopes of the two lines.";
  }

  if (topic.key === "parabola") {
    if (challengeParams.h !== targetParams.h || challengeParams.k !== targetParams.k) return "Find the vertex of the target parabola first.";
    return "The vertex is correct. Now look at the direction and width.";
  }

  if (topic.key === "circle") {
    if (challengeParams.h !== targetParams.h || challengeParams.k !== targetParams.k) return "Find the centre of the target circle first.";
    return "The centre is correct. Now compare the radius.";
  }

  if (topic.key === "nthroot") {
    if (challengeParams.h !== targetParams.h || challengeParams.k !== targetParams.k) return "Find the anchor point P(h, k) first.";
    if (challengeParams.n !== targetParams.n) return "Compare the domain and the shape: is the root index even or odd?";
    return "The position and root index are correct. Now compare the vertical scale and reflection.";
  }

  if (topic.key === "absolute") {
    if (challengeParams.h !== targetParams.h || challengeParams.k !== targetParams.k) return "Find the vertex of the target V-shape first.";
    return "The vertex is correct. Now compare the slopes of the two branches.";
  }

  if (topic.key === "exponential") {
    return "Look at the point where x = 1. Its y-coordinate is the base a.";
  }

  if (topic.key === "ellipse") {
    if (challengeParams.h !== targetParams.h || challengeParams.k !== targetParams.k) return "Find the centre of the target ellipse first.";
    if (challengeParams.a !== targetParams.a) return "Compare the horizontal semi-axis.";
    return "The centre and horizontal semi-axis are correct. Now compare the vertical semi-axis.";
  }
